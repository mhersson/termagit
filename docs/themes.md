...
- **`fg` vs `fg1`**: In most themes these are the same color. Set `fg1` to a
  slightly dimmer shade if you want normal body text to be subtler than bold/title
  text (like Tokyo Night does).
- **Reusing colors**: If your color scheme doesn't distinguish teal from cyan, or
  pink from purple, just set them to the same value. The built-in everforest-dark
  and tokyo-night themes do this.
